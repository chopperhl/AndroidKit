...
public class UpdateService extends Service {
    ...
    private static final String CHANNEL_NAME = "android-kit";
    private static final String CHANNEL_ID = BuildConfig.APPLICATION_ID;
    private static final String CHANNEL_DESC = BuildConfig.APPLICATION_ID;
    ...
    private static Intent installIntent(String path) {
        ...
        if (Build.VERSION.SDK_INT >= 24) {
            Uri apkUri = FileProvider.getUriForFile(BaseApplication.getApplication(), BaseApplication.getApplication().getPackageName()
                    + ".FileProvider", file);
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        if (!startDownload && intent != null) {
            ...
            downloadApkTask.executeOnExecutor(Executors.newSingleThreadExecutor(), downloadUrl);
        }
